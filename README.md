...
Everyone sees one current temple trip—there is no trip picker. Administrators can update its temple, date, and session time, replace it when needed, and remove names. The admin area is protected by a PIN so the regular member flow stays focused.
